...
int main()
{
    ...
    while(t--)
    {
        ll a,b;
        scanl2(a,b);
        // eq : x^2 -x -k=0 find zeroes for this and observe
        // it's tricky not difficult
        ll x=3,total=1,perfect=1; //we know 4 is the first one
        while(perfect*b > a*total)
        {
            if(isPowerofTwo(x))
            {
                perfect++;
            }
            trace3(x,perfect,total);
            total++;
            x++;
        }
        ll sol=x*(x-1);
