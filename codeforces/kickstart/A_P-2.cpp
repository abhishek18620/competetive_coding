...
bool dp[M][M];
char stemp1[M],stemp2[M],s1[M],s2[M],sol[10];

int expand(char *stemp , char *s)
{
...
int main()
{
    ...
    f(tt,1,t+1)
    {
        scanstr(stemp1); scanstr(stemp2);
        int l1 = expand(stemp1 , s1);
        int l2 = expand(stemp2 , s2);
        int ans=0;
        ...
        f(i,0,l1+1)
        {
            f(j,0,l2+1)
            {
                ...
                if(j<l2 and s2[j]=='*')
                    dp[i][j+1] = dp[i][j];
                if(i<l1 and j<l2 and (s1[i]==s2[j] or s1[i]=='*' or s2[j] =='*'))
                    dp[i+1][j+1]=dp[i][j];
            }
        }
        strcpy(sol , (dp[l1][l2])?"TRUE":"FALSE");
        printf("Case #%d: %s\n",tt,sol);
    }
