    ...
    f(row,0,m)
    {
        f(col,0,n)
        {
            if(!grid[row][col])
                continue;
            int maxH=m-row;
            maxH=min(maxH,min(col,n-col)+1);
            trace3(row,col,maxH);
            f(h,1,maxH+1)
            {
                if(isTriangle[row][col][h-1])
                {
                    bool poss=1;
                    f(y,col-h+1,col+h)
                    {
                    ...
                    isTriangle[row][col][h]=poss;
                    trace4(row,col,h,poss);
                }
            }
        }
    }
...
int main()
{
    ...
    f(tt,1,t+1)
    {
        ...
        memset(isTriangle,0,sizeof(isTriangle));
        f(i,0,m)
        {
            scanstr(str);
            f(j,0,n)
        ...
        checkTriangle();
        //dp begins
        //state(row at which k-1 tree ends +1, k trees)
        f(
        f(row,0,m)
        {
            f(trees,1,row+1)
