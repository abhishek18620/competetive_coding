// Q) Longest substring without repeating chars
/******************************************
...
int solve(string &s) {
  ...
  for (int i = 1; i < len; ++i) {
    char curr = s[i];
    int last_loc_i = last_loc[curr];
    if (last_loc_i == -1 || last_loc_i < i - maxLen[i - 1]) {
      // max substr is still growing
      maxLen[i] = maxLen[i - 1] + 1;
    } else {
      maxLen[i] = i - last_loc_i;
    }
    //trace4(curr, maxLen[i], maxLen[i - 1], last_loc_i);
    last_loc[curr] = i;
