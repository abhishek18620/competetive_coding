...
int find(char *str1,int len)
{
    node *parent=root;
    int cnt;
    f(i,0,len)
    {
        if(parent->child[str1[i]-'A']==NULL)
            return 0;
        else
        {
            parent=parent->child[str1[i]-'A'];
        }
    }
    return parent->cnt;
}

void addWord(char *str1 , int len)
{
    node *parent=root;
    f(i,0,len)
    {
        if(parent->child[str1[i]-'A']!=NULL)    //create new node
        {
            node *temp=new node;
            parent->child[str1[i]-'A']=temp;
            parent=temp;
            //parent->count++;
        }
        else
