...
bool CheckSubSeq(char *s1, char *s2, int x) //if s1^x is a subseq of s2
{
    char *news1=subseq(s1,x);
    int newlen1=strlen(news1);
    int len2=strlen(s2);
    int idx=0;
    trace3(news1,s2,x);
    f(i,0,len2)
    {
        if(s2[i]==news1[idx]) idx++;
    }
    if(idx==newlen1)
        return 1;
    return 0;
...
int main()
{
    ...
    while(t--)
    {
        scanstr(s1); scanstr(s2);
        int len1=strlen(s1);
        int len2=strlen(s2);
        int MaxPossPow=len2/len1;
        int low=1 , high=MaxPossPow;
        while(low<=high)
        {
            int mid=low+((high-low)>>1);
            if(CheckSubSeq(s1,s2,mid))
                low=mid+1;
            else
                high=mid-1;
        }
