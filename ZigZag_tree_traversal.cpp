 ...
 *  File : ZigZag_tree_traversal.cpp
 *******************************************/
...
void PrintZigZag(Node *root) {
  ...
  /* Stacks for storing nodes for currentlevel and nextlevel */
...
int main() {
  ...
  while(!node_queue.empty() and index <= 10) {
    Node* curr = node_queue.front();
    node_queue.pop();
    curr->left = new Node(2*index);
    curr->right= new Node(2*index+1);
    node_queue.push(curr->left);
    node_queue.push(curr->right);
    index ++;
  }

  // Call to function ZigZag_tree_traversal
  PrintZigZag(root);
  cout << endl;
#ifdef LOCAL_DEFINE
